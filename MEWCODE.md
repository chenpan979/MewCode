...
当前版本：`0.2.1`；要求 Python `3.11+`。

默认使用简体中文进行对话；当用户明确要求其他语言时，MewCode 会按要求切换。

### Windows 中文输入

Windows 下可在消息输入框中直接使用系统输入法：按 Shift 切换中英文，按 Ctrl+Shift 切换输入法。MewCode 会显示原生终端光标，使中文 IME 候选窗口跟随当前输入位置。
