...
class RemoteServer:
    ...
    def __init__(
        ...
        if not self._is_loopback_addr(addr) and not auth_token:
            raise ValueError(
                "Remote access outside localhost requires an authentication token"
            )
    ...
    def _is_loopback_addr(addr: str) -> bool:
        return addr.strip().lower() in {"127.0.0.1", "::1", "localhost"}

    def _is_authorized_request(self, request: Request, query: str) -> bool:
        ...
        if header.startswith("Bearer "):
            candidate = header[7:]
        if not candidate:
            candidate = parse_qs(query).get("token", [""])[0]
        return secrets.compare_digest(candidate, self.auth_token)
    ...
    async def run(self) -> None:
        ...
        display_host = "localhost" if self.addr in {"0.0.0.0", "::"} else self.addr
        if ":" in display_host and not display_host.startswith("["):
            display_host = f"[{display_host}]"
        remote_url = f"http://{display_host}:{self.port}/"
        if self.auth_token:
            remote_url += f"?token={quote(self.auth_token, safe='')}"
        print(f"\n  Remote UI: {remote_url}\n")
    ...
    def _process_http_request(
        ...
        if not self._has_allowed_origin(request):
            return Response(
                403,
                "Forbidden",
        ...
        if not self._is_authorized_request(request, parsed.query):
            return Response(
                401,
                "Unauthorized",
                websockets.Headers({"Content-Type": "text/plain; charset=utf-8"}),
