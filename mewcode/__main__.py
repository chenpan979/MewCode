...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Approve permission prompts in -p mode (default: deny)",
    )
    parser.add_argument(
        "--output-format",
    ...
    parser.add_argument(
        "--remote-host",
        default="127.0.0.1",
        help="Remote UI listen address (non-loopback requires --remote-token)",
    )
    parser.add_argument(
        "--remote-port",
        type=int,
        default=18888,
        help="Remote UI listen port (default: 18888)",
    )
    parser.add_argument(
        "--remote-token",
        default=os.environ.get("MEWCODE_REMOTE_TOKEN", ""),
        help="Remote UI authentication token (or set MEWCODE_REMOTE_TOKEN)",
    )
    ...
    if args.remote:
        ...
        try:
            server = RemoteServer(
                ...
                auth_token=args.remote_token,
            )
        except ValueError as e:
            print(f"Remote configuration error: {e}", file=sys.stderr)
            sys.exit(2)
