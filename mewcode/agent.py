...
class Agent:
    ...
    async def _run_pre_tool_hook(
        ...
        """执行统一的 pre-tool Hook；被拒绝时返回错误结果。"""
        if not self.hook_engine:
            return None
    ...
    async def _run_post_tool_hooks(self, tc: ToolCallComplete) -> None:
        """执行统一的 post-tool Hook。"""
        if not self.hook_engine:
            return
        hook_ctx = self._build_hook_context(
            "post_tool_use",
    ...
    async def _execute_single_tool_direct(
        ...
        hook_rejection = await self._run_pre_tool_hook(tc)
        ...
        await self._run_post_tool_hooks(tc)
    ...
    async def _execute_tool(
        ...
        if hook_rejection is not None:
            elapsed = time.monotonic() - start
            yield hook_rejection, elapsed, is_unknown
            return
