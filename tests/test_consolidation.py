...
def test_process_running_detects_current_process():
    from mewcode.memory.consolidation import _is_process_running
...
@pytest.mark.asyncio
async def test_consolidator_uses_current_tool_classes(tmp_path, monkeypatch):
    ...
    async def fake_run(self, conversation):
        captured["tools"] = {tool.name for tool in self.registry.list_tools()}
        captured["mode"] = self.permission_checker.mode
        if False:
            yield None
    ...
    assert captured["mode"] == PermissionMode.BYPASS
